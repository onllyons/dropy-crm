    ...
    <body class="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900">
        <div class="min-h-screen flex">
            ...
            <div class="flex-1 md:ml-64">
                <x-top-nav title="Poetry" />

                <main class="p-4 md:p-6">
                    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div>
                                <h1 class="text-2xl font-semibold">Poetry</h1>
                                <p class="mt-2 text-sm text-slate-600">Overview for read_poetry and activity.</p>
                            </div>
                    ...
                    <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div class="rounded-2xl border border-slate-200 bg-white p-4">
                            <div class="text-xs font-semibold text-slate-500">Total poems</div>
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-white p-4">
                            <div class="text-xs font-semibold text-slate-500">Poems with audio</div>
                            <div class="mt-2 text-xl font-semibold text-slate-700">{{ $summary['withAudio'] ?? 0 }}</div>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-white p-4">
                            <div class="text-xs font-semibold text-slate-500">Poems with subtitles</div>
                    ...
                    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div class="text-xs text-slate-500">Poems, bookmarks, reads by category.</div>
                        </div>
                    ...
                    <div class="mt-6 grid gap-4 lg:grid-cols-2">
                        <div class="rounded-2xl border border-slate-200 bg-white p-5">
                            <div class="text-sm font-semibold text-slate-700">Top poems by bookmarks</div>
                            <div class="mt-4 space-y-3">
                                @forelse ($topBookmarked as $poem)
                                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                        <div class="flex items-center justify-between gap-3">
                                            <div class="text-sm font-semibold text-slate-700">{{ $poem->title }}</div>
                                            <div class="text-xs font-semibold text-slate-600">Bookmarks: {{ $poem->count }}</div>
                                        </div>
                                        <div class="mt-1 text-xs text-slate-500">{{ $poem->url }}</div>
                                    </div>
                                @empty
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-white p-5">
                            <div class="text-sm font-semibold text-slate-700">Top poems by reads</div>
                            <div class="mt-4 space-y-3">
                                @forelse ($topRead as $poem)
                                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                        <div class="flex items-center justify-between gap-3">
                                            <div class="text-sm font-semibold text-slate-700">{{ $poem->title }}</div>
                                            <div class="text-xs font-semibold text-slate-600">Reads: {{ $poem->count }}</div>
                                        </div>
                                        <div class="mt-1 text-xs text-slate-500">{{ $poem->url }}</div>
                                    </div>
                                @empty
