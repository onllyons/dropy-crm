            ...
            <div class="flex-1 md:ml-64">
                ...
                <main class="p-4 md:p-6">
                    ...
                    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div>
                                <div class="text-sm font-semibold text-slate-700">Global user search (all time)</div>
                                <div class="text-xs text-slate-500">Search fields: name, username, email. Showing up to 100 rows.</div>
                            </div>
                            <form class="flex items-center gap-2" method="get" action="{{ url('/users') }}">
                                <input type="hidden" name="range" value="{{ $range ?? 'today' }}" />
                                <input type="hidden" name="q" value="{{ $search ?? '' }}" />
                                <input class="w-64 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-700 focus:border-slate-400 focus:outline-none" name="global_q" type="text" value="{{ $globalSearch }}" placeholder="Search all users by name, username, email" />
                                ...
                                @if ($globalSearch !== '')
                                    <a class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 hover:border-slate-300" href="{{ url('/users?range=' . ($range ?? 'today') . '&q=' . urlencode($search ?? '')) }}">Clear</a>
                                @endif
                            </form>
                        </div>
                                    ...
                                    @empty
                                        <tr>
                                            <td class="py-3 text-slate-500" colspan="6">
                                                {{ $globalSearch !== '' ? 'No users found in all-time search.' : 'Enter a search term to find users across all time.' }}
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                    ...
                    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-4">
                        @php
                            ...
                            $globalSearchParam = $globalSearch !== '' ? '&global_q=' . urlencode($globalSearch) : '';
                        @endphp
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div>
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                <form class="flex items-center gap-2" method="get" action="{{ url('/users') }}">
                                    ...
                                    <input type="hidden" name="global_q" value="{{ $globalSearch }}" />
                                    ...
                                    <button class="rounded-lg border border-slate-200 bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-800" type="submit">Search</button>
                                    @if ($search !== '')
                                        <a class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 hover:border-slate-300" href="{{ url('/users?range=' . $range . $globalSearchParam) }}">Clear</a>
                                    @endif
                                </form>
                                <div class="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2">
                                    <a class="{{ $rangeButtonBase }} {{ $range === 'today' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300' }}" href="{{ url('/users?range=today' . $searchParam . $globalSearchParam) }}">Today</a>
                                    <a class="{{ $rangeButtonBase }} {{ $range === 'yesterday' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300' }}" href="{{ url('/users?range=yesterday' . $searchParam . $globalSearchParam) }}">Yesterday</a>
                                    <a class="{{ $rangeButtonBase }} {{ $range === 'week' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300' }}" href="{{ url('/users?range=week' . $searchParam . $globalSearchParam) }}">This week</a>
                                    <a class="{{ $rangeButtonBase }} {{ $range === 'month' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300' }}" href="{{ url('/users?range=month' . $searchParam . $globalSearchParam) }}">This month</a>
                                </div>
                            </div>
                        </div>
                        <div class="mt-3 overflow-x-auto">
                            <table class="min-w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    ...
                                    @empty
                                        <tr>
                                            <td class="py-3 text-slate-500" colspan="6">No users found.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
