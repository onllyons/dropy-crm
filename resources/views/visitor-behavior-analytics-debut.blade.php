...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <x-seo-component title="visitorBehaviorAnalytics_debut" />
        <x-style-head-dropy />
        <x-styles-datatables />
    </head>
    <body class="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900">
        <div class="min-h-screen flex">
            ...
            <div class="flex-1 md:ml-64">
                <x-top-nav title="visitorBehaviorAnalytics_debut" />

                <main class="p-4 md:p-6">
                    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div>
                                <h1 class="text-2xl font-semibold">visitorBehaviorAnalytics_debut</h1>
                                <p class="mt-2 text-sm text-slate-600">Last {{ number_format($limit ?? 0) }} rows from visitorBehaviorAnalytics.</p>
                            </div>
                    ...
                    <div class="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
                        ...
                        <div class="mt-3 overflow-x-auto">
                            <table id="visitorAnalyticsDebutTable" class="min-w-full text-sm">
                                <thead>
        ...
        <script>
            $(document).ready(function () {
                $('#visitorAnalyticsDebutTable').DataTable({
                    dom: 'lBfrtip',
                    responsive: true,
                    columnDefs: [
